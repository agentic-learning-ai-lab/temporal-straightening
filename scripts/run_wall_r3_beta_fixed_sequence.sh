#!/usr/bin/env bash
# Fixed-direction R3 beta ladder on the Wall environment (training).
#
# Mirrors the umaze fixed-scale ladder (baseline_artifacts/analysis/
# umaze_r3_beta_fixed): every arm keeps the direction coefficient at SCALE
# (default 1e-1) and varies only the speed coefficient:
#
#     loss += SCALE * (r0 + beta * r1)
#     coefficient on r0 (direction) = SCALE          -- fixed at 0.1
#     coefficient on r1 (speed)     = SCALE * beta   -- the swept axis
#
# Wall conditions already on this axis (reused, NOT retrained here):
#
#     aggcos1e-1    r0 baseline        speed coeff 0    (wall_projector_on_paper_v4)
#     aggr2_5e-2    r2_full_matched    speed coeff 0.05 (wall_speed_ablations)
#     aggr3b1_1e-1  r3_beta1           speed coeff 0.1  (wall_speed_ablations)
#
# This script fills in the missing rungs, by default beta = 2 5 10
# (speed coefficients 0.2 / 0.5 / 1.0), into the same checkpoint root so
# planning and aggregation treat all conditions uniformly.
#
# Usage (from a JupyterHub terminal):
#   nohup bash scripts/run_wall_r3_beta_fixed_sequence.sh > logs/wall_r3_beta_fixed.nohup 2>&1 &
#   tail -f logs/wall_r3_beta_fixed.status
#
#   BETAS="2 5"                     # subset of arms
#   GPU_GROUPS="0,1;2,3"            # partly occupied node; group size must divide 32
#
# On a shared node, raise TRAINING_GPU_MAX_USED_MIB above whatever resident
# processes hold, or wait_for_gpus will block forever.
...
status_log="$PWD/logs/wall_r3_beta_fixed.status"
lock_file="$PWD/logs/wall_r3_beta_fixed.lock"
target_epochs="${TARGET_EPOCHS:-20}"
gpu_max_used_mib="${TRAINING_GPU_MAX_USED_MIB:-1024}"

# Fixed-scale mode on purpose: the direction coefficient stays at
# penalty_scale for every arm, so beta sweeps ONLY the speed coefficient.
# (The umaze sweep script's budget normalization is the other design; this
# ladder matches baseline_artifacts/analysis/umaze_r3_beta_fixed instead.)
penalty_scale="${PENALTY_SCALE:-1e-1}"

betas="${BETAS:-2 5 10}"

# Semicolon-separated GPU groups; one arm per group, concurrently. Group size
# must divide the effective batch size of 32 (1, 2, 4 or 8 GPUs per group).
gpu_groups="${GPU_GROUPS:-0,1,2,3;4,5,6,7}"
base_port="${BASE_PORT:-29810}"
...
echo "$(date -Is) LADDER_START betas=$betas direction_scale=$penalty_scale epochs=$target_epochs" >> "$status_log"
...
echo "$(date -Is) LADDER_COMPLETE betas=$betas" >> "$status_log"
